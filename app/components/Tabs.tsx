  ...
  const tabs = [
    {
      ...
      icon: (
        ...
        >
          <path
            d="M20.24 12.24a6 6 0 0 0-8.49-8.49L5 10.5V19h8.5l6.74-6.76zM16 17l-4-4"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
    ...
    <div className="flex items-center justify-center gap-4 p-4 bg-white rounded-lg shadow-sm">
      {tabs.map((tab) => (
        <Link
          ...
          href={tab.href}
          className={cn(
            "h-[44px] inline-flex items-center justify-center gap-3 px-5 text-sm font-medium rounded-lg transition-colors border border-transparent whitespace-nowrap",
            activeTab === tab.name
        ...
        >
          ...
          <span className="shrink-0">{tab.name}</span>
        </Link>
      ))}
    </div>
  );
